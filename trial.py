...
class HumanPlayer(AIPersonality):
    def lead_card(self, hand, trump_suit, trump_played):
        print(
            f"Your hand: {sorted(hand, key=lambda c: (Deck.suits.index(c.suit), Deck.ranks.index(c.rank)))}"
        )
        card_str = input("Choose a card to lead: ").strip()
    ...
    def follow_card(self, hand, led_suit, trump_suit, trump_played):
        ...
        card_str = input("Choose a card to play: ").strip()
        card = Card(card_str[0], card_str[1])
        hand.remove(card)
        return card
    ...
    def choose_trump_suit(self, hand):
        ...
        trump_suit = input("Choose a trump suit (♠, ♡, ♢, or ♣): ")
        return trump_suit


def create_personalities(num_players, human_player):
    personalities = [
        ConservativePlayer(),
        AggressivePlayer(),
        BalancedPlayer(),
    ...
    if human_player:
        ai_personalities = personalities[: num_players - 1]
        random.shuffle(ai_personalities)
        human_player_index = random.randint(0, num_players - 1)
        ai_personalities.insert(human_player_index, HumanPlayer())
...
def play_game(num_players=4, human_player=False):
    if human_player == False:
        ...
        for i, tricks in enumerate(tricks_won):
            if tricks == 0:
                scores[i] = -4
                print(
                    f"Player {i + 1} did not win any tricks, their score is set to -10"
                )
...
def main():
    ...
    print("Do you want to play as one of the players?")
    human_choice = input("Enter 'yes' or 'no': ").lower().strip()

    if human_choice == "yes":
        play_game(num_players, human_player=True)
    else:
        play_game(num_players, human_player=False)
